...
(async () => {
  ...
  app.get("/filteredImage", async (req, res) => {
    ...
    if (image_url) {
      if (!image_url.toString().match(image_url_regex)) {
        res.status(400).send("bad request, url not formated properly")
      } else {
        try {
          let image_response = await filterImageFromURL(image_url.toString())
          if (image_response != "no image found") {
            ...
            res.status(422).send("no image found in the given url")
          }
        } catch (err) {
          // other errors
          console.error(err)
